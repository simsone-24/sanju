...
export async function getById(req: Request, res: Response, next: NextFunction): Promise<void> {
  try {
    const actor = requireUser(req);
    const taskTemplate = await taskTemplatesService.getById(actor.companyId, parseId(req.params.id));
...
export async function update(
  ...
  try {
    const actor = requireUser(req);
    const taskTemplate = await taskTemplatesService.update(actor.companyId, actor.id, parseId(req.params.id), req.body);
...
export async function remove(req: Request<{ id: string }>, res: Response, next: NextFunction): Promise<void> {
  try {
    const actor = requireUser(req);
    await taskTemplatesService.remove(actor.companyId, actor.id, parseId(req.params.id));
